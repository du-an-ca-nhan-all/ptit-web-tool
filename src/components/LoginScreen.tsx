            ...
            {regSuccessMsg ? (
              <div className="flex flex-col items-center justify-center py-4 text-center gap-4">
                ...
                <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 text-xs text-slate-600 text-left w-full space-y-1">
                  <div>Mã SV: <b className="text-slate-900 font-mono">{regUsername}</b></div>
                  {studentInfo?.hoTen && <div>Họ tên: <b className="text-slate-900">{studentInfo.hoTen}</b></div>}
                  {studentInfo?.maLop && <div>Lớp: <b className="text-indigo-600 font-bold">{studentInfo.maLop}</b></div>}
                </div>
                <button
                  type="button"
                  onClick={() => {
