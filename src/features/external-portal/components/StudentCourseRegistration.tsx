...
export default function StudentCourseRegistration({
  ...
  const [selectedSemesterName, setSelectedSemesterName] = useState<string>('Đợt ĐKMH Hiện Tại');
  ...
  // 1. Fetch registered courses from database (supports semester filter)
  const fetchRegisteredCourses = async (semParam?: string) => {
    setIsLoading(true);
    try {
      ...
      const res = await fetch(
        `/api/course-registrations?username=${currentUser.username}&classCode=${currentUser.lop || ''}${semQuery}`
      );
      ...
      if (res.ok && data.success) {
        ...
        if (data.selectedSemester !== undefined) {
          setSelectedSemester(String(data.selectedSemester));
        }
        if (data.selectedSemesterName) {
          setSelectedSemesterName(data.selectedSemesterName);
        }
        setSummary({
  ...
  const handleSemesterChange = (newSem: string) => {
    ...
    fetchRegisteredCourses(newSem);
  };

  // 2. Active Pull from QLDTTX portal (pulls current + all past semesters)
  const handlePullFromQLDTTX = async () => {
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        await fetchRegisteredCourses(selectedSemester);
  ...
  const handleBatchPullClass = async () => {
    if (
      !confirm(
        `Bạn có chắc chắn muốn đồng bộ toàn bộ môn học và các kỳ cũ từ QLDTTX cho các sinh viên lớp ${currentUser.lop}?`
      )
    ) {
      return;
    }
  ...
  const filteredCourses = useMemo(() => {
    return courses.filter((item) => {
      ...
      const maMon = String(toHoc.ma_mon || item.ma_mon || '').toLowerCase();
      const tenMon = String(toHoc.ten_mon || item.ten_mon || '').toLowerCase();
      const nhomTo = String(toHoc.nhom_to || item.nhom_to || '').toLowerCase();
      const lop = String(toHoc.lop || item.lop || '').toLowerCase();
      const tkb = String(toHoc.tkb || item.tkb || '').toLowerCase();
      const gv = String(toHoc.gv || item.gv || '').toLowerCase();

      return (
        maMon.includes(query) ||
        tenMon.includes(query) ||
        nhomTo.includes(query) ||
        lop.includes(query) ||
        tkb.includes(query) ||
        gv.includes(query)
      );
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h1 className="text-xl sm:text-2xl font-black text-slate-800 tracking-tight">
                  Môn Học Đã Đăng Ký (ĐKMH & TKB Học Kỳ)
                </h1>
                ...
                {semesters.length > 0 && (
                  <span className="bg-emerald-100 text-emerald-800 border border-emerald-300 text-[10px] font-bold px-2 py-0.5 rounded-full">
                    {semesters.length} Học Kỳ Đã Lưu
                  </span>
                )}
              </div>
              <p className="text-slate-500 text-xs sm:text-sm mt-0.5">
                Sinh viên: <strong className="text-indigo-600 font-mono">{currentUser.fullName || currentUser.username}</strong> ({currentUser.username}) • Lớp:{' '}
                <strong className="text-blue-600">{currentUser.lop || 'Chưa phân lớp'}</strong>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap w-full md:w-auto">
          ...
          <button
            ...
            title="Đồng bộ toàn bộ môn học kỳ hiện tại và các kỳ cũ trực tiếp từ cổng QLDTTX (/#/tkb-hocky)"
          >
            {isPulling ? (
            ...
            <span>Đồng Bộ Từ QLDTTX (Gồm Kỳ Cũ)</span>
          </button>
          ...
          {(isAdmin || isMonitor) && currentUser.lop && (
            <button
              ...
              title="Kéo toàn bộ môn học và các kỳ cũ cho tất cả sinh viên trong lớp"
            >
              {isBatchPulling ? (
                <div className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
              ) : (
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl p-1.5 border border-slate-200 shadow-xs flex items-center gap-2 overflow-x-auto scrollbar-none">
        ...
        >
          ...
          <span>So Sánh Với Lớp Trưởng</span>
          <span
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-amber-800 mt-0.5">
                    Để hệ thống tự động kéo kết quả đăng ký môn học và các kỳ cũ từ cổng trường (https://qldttx.pttc1.edu.vn/), vui lòng liên kết tài khoản trong phần Hồ Sơ.
                  </p>
                </div>
              </div>
          ...
          <div className="bg-white rounded-3xl p-4 sm:p-5 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[11px] text-slate-400">
                  Lọc danh sách môn học theo từng học kỳ từ cổng QLDTTX (/#/tkb-hocky)
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <select
                ...
                className="w-full sm:w-auto bg-slate-50 border border-slate-300 rounded-2xl px-4 py-2.5 text-xs font-bold text-slate-800 outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer min-w-[260px]"
              >
                <option value="CURRENT">⚡ Đợt ĐKMH Hiện Tại</option>
                {semesters.map((s) => (
                  <option key={s.hoc_ky} value={String(s.hoc_ky)}>
                    📅 {s.ten_hoc_ky} ({s.totalCourses} môn - {s.totalCredits} TC)
                  </option>
                ))}
                {semesters.length > 0 && <option value="ALL">🌐 Tất Cả Các Học Kỳ Đã Học</option>}
              </select>
            </div>
          </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-2xl font-black text-slate-800 mt-0.5">
                  {summary.totalCourses} <span className="text-xs font-normal text-slate-400">môn học</span>
                </div>
                <div className="text-[11px] text-blue-600 font-bold mt-0.5 truncate max-w-[180px]">{selectedSemesterName}</div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-2xl font-black text-indigo-600 mt-0.5">
                  {summary.totalCredits} <span className="text-xs font-normal text-slate-400">tín chỉ (TC)</span>
                </div>
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                ...
                <div className="text-xl font-black text-emerald-700 mt-0.5">
                  {summary.tuitionFee > 0 ? `${summary.tuitionFee.toLocaleString('vi-VN')} đ` : 'Đã thanh toán / TKB'}
                </div>
                <div className="text-[11px] text-emerald-700 font-bold mt-0.5">Theo cổng PTTC1</div>
              </div>
            </div>
            ...
            <div className="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-50/50">
              <div className="relative w-full sm:max-w-md">
                ...
                <input
                  ...
                  placeholder="Tìm theo tên môn, mã môn, nhóm tổ, TKB, giảng viên..."
              ...
              <div className="text-xs text-slate-500 font-bold self-end sm:self-center">
                Hiển thị: <strong className="text-blue-600">{filteredCourses.length}</strong> / {courses.length} môn học ({selectedSemesterName})
              </div>
            </div>
            ...
            <div className="overflow-x-auto">
              {isLoading ? (
                <div className="py-20 flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-xs text-slate-500 font-bold">Đang tải danh sách môn học...</p>
                </div>
                ...
                <div className="py-20 text-center flex flex-col items-center justify-center gap-3">
                  ...
                  <p className="text-sm font-bold text-slate-700">Chưa có dữ liệu môn học cho học kỳ này</p>
                  <p className="text-xs text-slate-400 max-w-sm">
                    {summary.externalAccount?.isConfigured
                      ? 'Bấm nút "Đồng Bộ Từ QLDTTX (Gồm Kỳ Cũ)" phía trên để kéo toàn bộ môn học của các kỳ về máy.'
                  ...
                  {summary.externalAccount?.isConfigured && (
                    ...
                    >
                      ...
                      <span>Đồng Bộ Toàn Bộ Kỳ Ngay</span>
                    </button>
                  )}
                </div>
              ) : (
                <table className="w-full text-left text-xs">
                  <thead className="bg-slate-50 text-slate-600 font-bold uppercase tracking-wider border-b border-slate-200">
                    <tr>
                      ...
                      <th className="px-4 py-3.5">Giảng Viên / Phòng</th>
                      <th className="px-4 py-3.5 text-center">Học Online</th>
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {filteredCourses.map((item, index) => {
                      const toHoc = item.to_hoc || item || {};
                      const maMon = toHoc.ma_mon || item.ma_mon;
                      const tenMon = toHoc.ten_mon || item.ten_mon;
                      const soTC = toHoc.so_tc || toHoc.so_tc_hp || item.so_tc || 0;
                      const nhomTo = toHoc.nhom_to || item.nhom_to || '—';
                      const lop = toHoc.lop || item.lop || toHoc.ten_lop || '—';
                      const tkb = toHoc.tkb || item.tkb || '';
                      const phong = toHoc.phong || item.phong || '';
                      const gv = toHoc.gv || item.gv || '';
                      const dtGv = toHoc.dt_gv || item.dt_gv || '';
                      const linkOnline = toHoc.link_hoc_online || item.link_hoc_online || '';

                      return (
                        <tr
                          key={item.id_kqdk || item.id_to_hoc || index}
                        ...
                        >
                          ...
                          <td className="px-4 py-3.5">
                            <span className="font-bold text-slate-800 text-sm">{tenMon}</span>
                            {item.semesterName && selectedSemester === 'ALL' && (
                              <span className="ml-2 bg-slate-100 text-slate-600 px-2 py-0.5 rounded text-[10px] font-bold">
                                {item.semesterName}
                              </span>
                            )}
                            {item.ngay_dang_ky && (
                          ...
                          <td className="px-4 py-3.5 text-slate-600 font-mono text-[11px] max-w-[150px] truncate" title={lop}>
                            {lop}
                          </td>
                          <td className="px-4 py-3.5 max-w-xs">
                            {tkb ? (
                              <div
                                ...
                                dangerouslySetInnerHTML={{ __html: tkb }}
                              />
                            ) : (
                          ...
                          <td className="px-4 py-3.5 text-xs text-slate-700">
                            {gv ? (
                              <div>
                                <span className="font-bold block">{gv}</span>
                                <div className="text-[11px] text-slate-400 flex items-center gap-1 mt-0.5">
                                  {phong && <span>{phong}</span>}
                                  {dtGv && <span>• {dtGv}</span>}
                                </div>
                              </div>
                            ) : (
                              <span className="text-slate-400 italic text-[11px]">{phong || 'Chưa phân công'}</span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 text-center">
                            {linkOnline ? (
                              <a
                                ...
                                className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg text-[10px] font-bold transition-colors cursor-pointer"
                                title="Vào phòng học Zoom"
                              >
          ...
          {selectedCourseDetail && (
            ...
            >
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-200">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-lg font-black tracking-tight">
                        {selectedCourseDetail.to_hoc?.ten_mon || selectedCourseDetail.ten_mon}
                      </h3>
                      <p className="text-xs text-blue-100 font-mono mt-0.5">
                        Mã môn: {selectedCourseDetail.to_hoc?.ma_mon || selectedCourseDetail.ma_mon} • Nhóm tổ:{' '}
                        {selectedCourseDetail.to_hoc?.nhom_to || selectedCourseDetail.nhom_to}
                      </p>
                    </div>
                  </div>
                  <button
                ...
                <div className="p-6 flex flex-col gap-4 text-xs text-slate-700 max-h-[75vh] overflow-y-auto">
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
                      <span className="text-slate-400 font-bold block mb-0.5">Số tín chỉ:</span>
                      <span className="font-bold text-slate-800 text-sm">
                        {selectedCourseDetail.to_hoc?.so_tc ||
                          selectedCourseDetail.to_hoc?.so_tc_hp ||
                          selectedCourseDetail.so_tc}{' '}
                        TC
                      </span>
                    </div>

                    <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200">
                      <span className="text-slate-400 font-bold block mb-0.5">Lớp học phần:</span>
                      <span className="font-bold text-indigo-700 text-xs font-mono">
                        {selectedCourseDetail.to_hoc?.lop || selectedCourseDetail.lop || '—'}
                      </span>
                    </div>
                  </div>

                  {/* Lecturer details */}
                  {(selectedCourseDetail.to_hoc?.gv || selectedCourseDetail.gv) && (
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200 flex items-center justify-between">
                      <div>
                        <span className="text-slate-400 font-bold block mb-0.5">Giảng viên phụ trách:</span>
                        <span className="font-bold text-slate-800 text-sm">
                          {selectedCourseDetail.to_hoc?.gv || selectedCourseDetail.gv}
                        </span>
                        {(selectedCourseDetail.to_hoc?.phong || selectedCourseDetail.phong) && (
                          <div className="text-[11px] text-slate-500 mt-0.5">
                            Phòng học: {selectedCourseDetail.to_hoc?.phong || selectedCourseDetail.phong}
                          </div>
                        )}
                      </div>

                      {(selectedCourseDetail.to_hoc?.dt_gv || selectedCourseDetail.dt_gv) && (
                        <a
                          href={`tel:${selectedCourseDetail.to_hoc?.dt_gv || selectedCourseDetail.dt_gv}`}
                          className="px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold rounded-xl border border-indigo-200 flex items-center gap-1.5"
                        >
                          <Phone className="w-3.5 h-3.5" />
                          <span>{selectedCourseDetail.to_hoc?.dt_gv || selectedCourseDetail.dt_gv}</span>
                        </a>
                      )}
                    </div>
                  )}
                  ...
                  {/* Online class link */}
                  {(selectedCourseDetail.to_hoc?.link_hoc_online || selectedCourseDetail.link_hoc_online) && (
                    <div className="bg-blue-50 p-4 rounded-2xl border border-blue-200 flex items-center justify-between">
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <span className="text-blue-900 font-bold block">Phòng Học Trực Tuyến (Zoom):</span>
                          <span className="text-blue-700 text-[11px] font-mono truncate block max-w-[240px]">
                            {selectedCourseDetail.to_hoc?.link_hoc_online || selectedCourseDetail.link_hoc_online}
                          </span>
                        </div>
                      </div>
                      ...
                      <a
                        href={selectedCourseDetail.to_hoc?.link_hoc_online || selectedCourseDetail.link_hoc_online}
                        target="_blank"
                        rel="noreferrer"
                        className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-sm shrink-0"
                      >
                        Tham Gia →
                      </a>
                    </div>
                  ...
                  {/* Schedule details */}
                  {(selectedCourseDetail.to_hoc?.tkb || selectedCourseDetail.tkb) && (
                    <div className="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                      <span className="text-slate-800 font-bold block mb-1 flex items-center gap-1.5">
                      ...
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: selectedCourseDetail.to_hoc?.tkb || selectedCourseDetail.tkb,
                        }}
                      />
                    </div>
                  )}
