...
export default function ActivityLogsManager({ currentUser }: ActivityLogsManagerProps) {
  ...
  return (
    <div className="flex-1 flex flex-col p-3 sm:p-5 md:p-8 max-w-7xl mx-auto w-full gap-4 sm:gap-6 animate-in fade-in duration-300">
      {/* Top Banner Header */}
      <div className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white p-4 sm:p-6 md:p-8 rounded-2xl sm:rounded-3xl shadow-xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3 sm:gap-4 relative z-10">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="px-2 py-0.5 sm:px-2.5 bg-indigo-500/20 text-indigo-300 border border-indigo-500/40 text-[10px] sm:text-xs font-bold rounded-full">
                Audit Logs
              </span>
            </div>
            <p className="text-[11px] sm:text-xs text-slate-400 mt-0.5 sm:mt-1 leading-snug">
              Lưu vết toàn bộ thao tác: đăng nhập, đổi vai trò, giả lập, đồng bộ và chỉnh sửa dữ liệu
            </p>
          </div>
        </div>

        {/* Action Buttons / Total Count (2-col on mobile, flex on desktop) */}
        <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 sm:gap-3 relative z-10 shrink-0 w-full md:w-auto">
          <div className="bg-white/5 backdrop-blur-md px-3 py-2 sm:px-4 sm:py-2 rounded-xl sm:rounded-2xl border border-white/10 text-center flex flex-col justify-center">
            <div className="text-[9px] sm:text-[10px] text-slate-400 font-bold uppercase tracking-wider">Tổng Bản Ghi</div>
            <div className="text-base sm:text-lg font-black text-white">{pagination.total.toLocaleString('vi-VN')}</div>
          </div>
          ...
          {isAdmin && (
            <button
              ...
              className="px-3 sm:px-4 py-2.5 sm:py-3 bg-rose-600 hover:bg-rose-700 active:bg-rose-800 text-white text-xs font-bold rounded-xl sm:rounded-2xl transition-all shadow-md shadow-rose-600/30 flex items-center justify-center gap-1.5 cursor-pointer border border-rose-500/40"
              title="Dọn dẹp và xoá bớt nhật ký hoạt động"
            >
          ...
          <button
            ...
            className="col-span-2 sm:col-span-1 px-3 sm:px-4 py-2 sm:py-3 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white text-xs font-bold rounded-xl sm:rounded-2xl transition-all shadow-md shadow-indigo-600/30 flex items-center justify-center gap-1.5 cursor-pointer"
        ...
        {Object.entries(ACTION_CATEGORIES).map(([key, cat]) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 sm:px-3.5 sm:py-2 rounded-xl sm:rounded-2xl text-[11px] sm:text-xs font-bold whitespace-nowrap transition-all cursor-pointer border shrink-0 ${
                isSelected
                  ? 'bg-indigo-600 text-white border-indigo-600 shadow-xs'
                  : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50 hover:text-slate-900 active:bg-slate-100'
              }`}
            >
              {cat.label}
            </button>
      ...
      {/* Filter & Search Bar */}
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2.5 sm:gap-4">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-3 w-full md:w-auto flex-1">
          ...
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Tìm theo mô tả, người dùng, mã SV..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9.5 pr-4 py-2 sm:py-2.5 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl text-xs font-medium text-slate-800 placeholder-slate-400 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
            />
          </div>
          ...
          <div className="relative shrink-0 w-full sm:w-auto">
            <select
              ...
              className="w-full sm:w-auto pl-3 pr-8 py-2 sm:py-2.5 bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl text-xs font-bold text-slate-700 focus:bg-white focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer appearance-none truncate"
            >
              <option value="">
        ...
        {/* Pagination Summary */}
        <div className="flex items-center gap-2 text-xs text-slate-500 font-bold justify-between md:justify-end shrink-0 pt-1 sm:pt-0 border-t sm:border-t-0 border-slate-100">
          <span>
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="p-1.5 sm:p-2 rounded-lg sm:rounded-xl border border-slate-200 hover:bg-slate-50 active:bg-slate-100 disabled:opacity-30 disabled:cursor-not-allowed cursor-pointer text-slate-700"
            >
      ...
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        {isLoading ? (
          ...
          <div className="py-16 sm:py-20 flex flex-col items-center justify-center text-center p-4 gap-3">
            <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl sm:rounded-3xl bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-400">
              <Info className="w-6 h-6" />
            </div>
            <h3 className="text-sm sm:text-base font-bold text-slate-800">Chưa có nhật ký hoạt động phù hợp</h3>
          ...
          <>
            ...
            {/* ========================================================================= */}
            <div className="block md:hidden divide-y divide-slate-100 p-2.5 sm:p-4 space-y-2.5">
              {displayedLogs.map((log) => {
                ...
                return (
                  <div
                    key={log.id}
                    className="bg-white rounded-2xl border border-slate-200/90 p-3 shadow-2xs space-y-2.5 hover:border-indigo-200 transition-all"
                  >
                    {/* Header: Action badge and Time */}
                    <div className="flex items-start justify-between gap-2">
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold border ${actConfig.bg} ${actConfig.text} ${actConfig.border}`}
                      >
                        <ActIcon className="w-3 h-3 shrink-0" />
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100">
                      ...
                      {log.targetId && (
                        <div className="shrink-0 text-right">
                          <span className="inline-flex items-center bg-slate-100 text-slate-700 px-1.5 py-0.5 rounded text-[10px] font-mono font-bold">
                            {log.targetId}
                          </span>
                        </div>
                      )}
                    ...
                    {/* Card Footer: IP / Device & Detail Button */}
                    <div className="flex items-center justify-between gap-2 pt-1.5 border-t border-slate-100">
                      <span
                        className="font-mono text-[10px] text-slate-400 bg-slate-50 px-2 py-0.5 rounded border border-slate-100 truncate max-w-[150px]"
                        title={log.userAgent || ''}
                      >
                      ...
                      <button
                        ...
                        className="px-2.5 py-1 bg-indigo-50 hover:bg-indigo-100 active:bg-indigo-200 text-indigo-700 rounded-xl text-[11px] font-bold transition-all cursor-pointer inline-flex items-center gap-1 shadow-2xs border border-indigo-200/60"
                      >
        ...
        {/* Footer Pagination */}
        {logs.length > 0 && (
          <div className="p-3 sm:p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-2.5 text-xs text-slate-600 font-bold">
            <div>
              Hiển thị {logs.length} / {pagination.total.toLocaleString('vi-VN')} bản ghi hoạt động
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => fetchLogs(pagination.page - 1)}
                disabled={pagination.page <= 1 || isLoading}
                className="px-3 py-1.5 bg-white border border-slate-200 rounded-xl hover:bg-slate-100 active:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer shadow-xs"
              >
                Trang Trước
              </button>
              <span>
      ...
      {selectedLogMetadata && (() => {
        ...
        return (
          <div
            className="fixed inset-0 bg-slate-950/70 backdrop-blur-md flex items-center justify-center z-50 p-3 sm:p-4 animate-in fade-in duration-200"
            onClick={(e) => {
              if (e.target === e.currentTarget) setSelectedLogMetadata(null);
            }}
          >
            <div className="bg-white rounded-2xl sm:rounded-3xl max-w-2xl w-full shadow-2xl overflow-hidden border border-slate-200 flex flex-col animate-in zoom-in-95 duration-200 max-h-[92vh]">
              {/* Modal Header */}
              <div className="p-4 sm:p-5 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white flex items-center justify-between border-b border-slate-800 shrink-0">
                <div className="flex items-center gap-3 min-w-0">
                  <div className={`p-2 sm:p-2.5 rounded-xl sm:rounded-2xl border ${modalConfig.bg} ${modalConfig.text} ${modalConfig.border} shrink-0`}>
                    <ModalActIcon className="w-4 h-4 sm:w-5 sm:h-5" />
                  </div>
                  <div className="min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <h4 className="font-black text-sm sm:text-base tracking-tight text-white truncate">{modalConfig.label}</h4>
                    ...
                    <p className="text-[10px] sm:text-[11px] text-slate-400 mt-0.5 truncate">
                      ID: <span className="font-mono text-indigo-300 font-bold">#{selectedLogMetadata.id}</span> • Phân loại:{' '}
                ...
                <button
                  ...
                  className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/10 hover:bg-white/20 text-slate-300 hover:text-white flex items-center justify-center transition-all cursor-pointer text-sm font-bold shrink-0 ml-2"
                >
                  ✕
                </button>
              </div>
              ...
              <div className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 overflow-y-auto">
                {/* Description Banner */}
                <div className="bg-gradient-to-br from-indigo-50/70 via-slate-50 to-blue-50/50 p-3.5 sm:p-4 rounded-2xl border border-indigo-100/80 flex flex-col gap-1 shadow-2xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3 text-xs">
                  ...
                  <div className="bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-200/80 flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-slate-200 overflow-hidden ring-1 ring-slate-300 shrink-0">
                      <img
                  ...
                  {/* Target */}
                  <div className="bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-200/80 flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-indigo-50 border border-indigo-200/60 flex items-center justify-center text-indigo-600 shrink-0">
                  ...
                  {/* Time */}
                  <div className="bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-200/80 flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-emerald-50 border border-emerald-200/60 flex items-center justify-center text-emerald-600 shrink-0">
                  ...
                  {/* Network / IP */}
                  <div className="bg-slate-50 p-3 sm:p-3.5 rounded-2xl border border-slate-200/80 flex items-center gap-3">
                    <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-cyan-50 border border-cyan-200/60 flex items-center justify-center text-cyan-600 shrink-0">
                ...
                <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {hasMetadata && (
                        <span className="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-200 text-[10px] font-bold rounded-md">
                          {Object.keys(metaObj).length} trường
                        </span>
                      )}
                    </div>
                  ...
                  {!hasMetadata ? (
                    <div className="p-5 sm:p-6 bg-slate-50 rounded-2xl border border-slate-200 text-center flex flex-col items-center justify-center gap-2 text-slate-400">
                      <Info className="w-5 h-5 sm:w-6 sm:h-6 text-slate-300" />
                    ...
                    <div className="relative">
                      <pre className="bg-slate-900 text-emerald-300 p-3.5 sm:p-4 rounded-2xl text-[11px] font-mono overflow-x-auto max-h-64 leading-relaxed border border-slate-800 shadow-inner">
                ...
                {selectedLogMetadata.userAgent && (
                  <div>
                    ...
                    <p className="text-[10px] sm:text-[11px] text-slate-600 font-mono bg-slate-50 p-2.5 sm:p-3 rounded-2xl border border-slate-200 break-all leading-relaxed">
                      {selectedLogMetadata.userAgent}
                    </p>
                  </div>
                )}
              ...
              <div className="p-3.5 sm:p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between shrink-0">
                <div className="text-[10px] sm:text-[11px] text-slate-400 font-medium truncate">
                  #{selectedLogMetadata.id} • {modalConfig.label}
                </div>
                <button
                  type="button"
                  onClick={() => setSelectedLogMetadata(null)}
                  className="px-5 sm:px-6 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl sm:rounded-2xl text-xs font-bold transition-all shadow-md cursor-pointer"
                >
                  Đóng
                </button>
              </div>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-200 animate-in zoom-in-95 duration-200 max-h-[92vh] flex flex-col">
            {/* Modal Header */}
            <div className="bg-gradient-to-r from-rose-900 via-slate-900 to-rose-950 text-white p-4 sm:p-5 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2.5">
                <div className="p-2 bg-rose-500/30 text-rose-300 rounded-xl border border-rose-400/30 shrink-0">
                  <Trash2 className="w-4 h-4 sm:w-5 sm:h-5" />
                </div>
                <div>
                  <h4 className="font-bold text-xs sm:text-sm">Dọn Dẹp & Xoá Nhật Ký Hoạt Động</h4>
                  <p className="text-[10px] sm:text-[11px] text-rose-200/80">
            ...
            <div className="p-4 sm:p-6 flex flex-col gap-4 sm:gap-5 overflow-y-auto">
              {/* Alert Feedback */}
              {deleteSuccessMsg && (
                <div className="p-3.5 sm:p-4 bg-emerald-50 border border-emerald-200 rounded-2xl flex items-center gap-2.5 text-emerald-900 text-xs font-bold animate-in fade-in">
              ...
              {deleteErrorMsg && (
                <div className="p-3.5 sm:p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-center gap-2.5 text-rose-900 text-xs font-bold animate-in fade-in">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
                  {[
                    ...
                    { id: 'ALL', label: 'Xoá Tất Cả', desc: 'Toàn bộ log' },
                  ].map((tab) => (
                    <button
                      key={tab.id}
                      type="button"
                      ...
                      className={`p-2.5 sm:p-3 rounded-xl sm:rounded-2xl text-left border transition-all cursor-pointer flex flex-col gap-0.5 ${
                        deleteMode === tab.id
                          ? tab.id === 'ALL'
                    ...
                    >
                      ...
                      <span className="text-[9px] sm:text-[10px] opacity-75">{tab.desc}</span>
                    </button>
                  ))}
                </div>
              </div>
              ...
              {deleteMode === 'COUNT' && (
                <div className="bg-slate-50 p-3.5 sm:p-4 rounded-2xl border border-slate-200 flex flex-col gap-3">
              ...
              {deleteMode === 'ALL' && (
                <div className="bg-rose-50 p-3.5 sm:p-4 rounded-2xl border border-rose-300 flex flex-col gap-3">
                  <div className="flex items-center gap-2 text-rose-800 font-bold text-xs">
                    <AlertTriangle className="w-4 h-4 text-rose-600 shrink-0" />
                    <span>CẢNH BÁO NGUY HIỂM: XOÁ TOÀN BỘ NHẬT KÝ</span>
                  </div>

                  <p className="text-[10px] sm:text-[11px] text-rose-900 leading-relaxed">
                  ...
                  <div className="pt-2 border-t border-rose-200">
                    <label className="block text-[10px] sm:text-[11px] font-bold text-rose-800 mb-1">
              ...
              <label className="flex items-start gap-2.5 p-3 rounded-2xl bg-amber-50/70 border border-amber-200 cursor-pointer">
                ...
                <div className="text-xs">
                  ...
                  <p className="text-[10px] sm:text-[11px] text-amber-800/80 mt-0.5 leading-relaxed">
            ...
            {/* Modal Actions */}
            <div className="p-3.5 sm:p-4 bg-slate-50 border-t border-slate-200 flex items-center justify-between gap-2 sm:gap-3 shrink-0">
              <button
                type="button"
                onClick={() => setShowDeleteModal(false)}
                disabled={isDeleting}
                className="px-4 sm:px-5 py-2 sm:py-2.5 bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 rounded-xl text-xs font-bold transition-colors cursor-pointer disabled:opacity-50"
              >
                Hủy Bỏ
              </button>
              ...
              <button
                ...
                className={`px-4 sm:px-6 py-2 sm:py-2.5 text-white rounded-xl text-xs font-bold transition-all shadow-md flex items-center justify-center gap-1.5 sm:gap-2 cursor-pointer disabled:opacity-50 ${
                  deleteMode === 'ALL'
                  ...
                  <>
                    ...
                    <span className="truncate">
                      {deleteMode === 'ALL'
                        ? 'Xác Nhận Xoá Tất Cả'
                        : deleteMode === 'COUNT'
                        ? `Xoá ${customCountInput ? parseInt(customCountInput, 10) || 0 : deleteCount} Log Cũ`
                        : `Xoá Log Cũ Hơn ${deleteDays} Ngày`}
                    </span>
                  </>
                )}
              </button>
