  ...
  return (
    <div className="space-y-4 sm:space-y-6 animate-in fade-in duration-200">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-2.5 sm:gap-3.5">
        {/* Queued */}
        <div className="bg-white p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-[10px] sm:text-[11px] font-bold uppercase">Chờ</span>
            <Clock className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-amber-600 mt-0.5 sm:mt-1">{stats.totalQueued}</div>
          <div className="text-[9px] sm:text-[10px] text-slate-400 font-bold mt-0.5 hidden sm:block">Sẵn sàng chạy lần lượt</div>
        </div>

        {/* Running */}
        <div className="bg-white p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-[10px] sm:text-[11px] font-bold uppercase">Đang chạy</span>
            <RefreshCw className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-indigo-600 ${isWorkerRunning ? 'animate-spin' : ''}`} />
          </div>
          <div className="text-xl sm:text-2xl font-black text-indigo-600 mt-0.5 sm:mt-1">{stats.totalRunning}</div>
          <div className="text-[9px] sm:text-[10px] text-indigo-600 font-bold mt-0.5 hidden sm:block">
        ...
        <div className="bg-white p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-[10px] sm:text-[11px] font-bold uppercase">Thành công</span>
            <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-emerald-600" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-emerald-700 mt-0.5 sm:mt-1">{stats.totalSuccess}</div>
          <div className="text-[9px] sm:text-[10px] text-emerald-600 font-bold mt-0.5 hidden sm:block">Đã xong</div>
        </div>

        {/* Failed */}
        <div className="bg-white p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-[10px] sm:text-[11px] font-bold uppercase">Lỗi</span>
            <XCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-rose-600" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-rose-600 mt-0.5 sm:mt-1">{stats.totalFailed}</div>
          <div className="text-[9px] sm:text-[10px] text-rose-600 font-bold mt-0.5 hidden sm:block">
            {stats.totalFailed > 0 ? 'Cần Thử Lại' : '0 lỗi'}
          </div>
        </div>

        {/* Cancelled / Skipped */}
        <div className="bg-white p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm col-span-2 sm:col-span-1">
          <div className="flex items-center justify-between">
            <span className="text-slate-400 text-[10px] sm:text-[11px] font-bold uppercase">Đã Hủy</span>
            <AlertTriangle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-purple-600" />
          </div>
          <div className="text-xl sm:text-2xl font-black text-purple-600 mt-0.5 sm:mt-1">{stats.totalCancelled}</div>
          <div className="text-[9px] sm:text-[10px] text-purple-600 font-bold mt-0.5 hidden sm:block">Đã hủy/bỏ qua</div>
        </div>
      </div>
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <h3 className="font-black text-sm sm:text-base text-white">{activeBatch.title}</h3>
              </div>
              <p className="text-[10px] sm:text-xs text-slate-400 mt-1">
                Batch: <span className="font-mono text-slate-300">{activeBatch.id}</span> • {' '}
            ...
            <div className="grid grid-cols-2 sm:flex sm:items-center gap-2 w-full sm:w-auto">
              ...
              {stats.totalFailed > 0 && (
                <button
                  ...
                  disabled={isSubmitting}
                  className="px-3 py-2 sm:px-3.5 bg-amber-500 hover:bg-amber-600 text-slate-900 text-xs font-black rounded-xl sm:rounded-2xl transition-colors flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
                  title="Chạy lại tất cả các tác vụ bị lỗi"
                >
                  <RotateCcw className="w-3.5 h-3.5" />
                  <span>Thử Lại ({stats.totalFailed})</span>
                </button>
              )}
              ...
              <button
                ...
                disabled={isLoading}
                className="px-3 py-2 sm:px-3.5 bg-white/10 hover:bg-white/20 text-white text-xs font-bold rounded-xl sm:rounded-2xl transition-colors border border-white/20 flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
              ...
              <button
                ...
                className="px-3 py-2 sm:px-3.5 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-xl sm:rounded-2xl transition-colors border border-slate-700 flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50 col-span-2 sm:col-span-1"
      ...
      {batches.length > 1 && (
        <div className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto pb-1 scroll-smooth snap-x">
          <span className="text-[11px] sm:text-xs font-bold text-slate-400 uppercase shrink-0">Các đợt:</span>
          <button
            onClick={() => setSelectedBatchId(null)}
            className={`px-3 py-1.5 rounded-xl text-xs font-bold shrink-0 snap-start transition-all cursor-pointer ${
              !selectedBatchId
          ...
          {batches.map((b) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-bold shrink-0 snap-start transition-all cursor-pointer flex items-center gap-1.5 ${
            ...
            >
              ...
              />
              <span className="truncate max-w-[120px] sm:max-w-[180px]">{b.title}</span>
      ...
      {/* Live Queue Items Table & Mobile Cards */}
      <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        ...
        <div className="p-3.5 sm:p-5 border-b border-slate-100 flex flex-col md:flex-row items-center justify-between gap-3 sm:gap-4 bg-slate-50/50">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 overflow-x-auto w-full md:w-auto justify-start md:justify-end snap-x scroll-smooth">
            <button
              onClick={() => setStatusFilter('ALL')}
              className={`px-2.5 sm:px-3 py-1 rounded-xl text-[11px] sm:text-xs font-bold transition-all cursor-pointer shrink-0 snap-start ${
                statusFilter === 'ALL'
                  ? 'bg-slate-900 text-white shadow-xs'
        ...
        {isLoading ? (
          ...
          <div className="py-16 sm:py-20 text-center flex flex-col items-center justify-center gap-2 text-slate-400">
          ...
          <>
            ...
            <div className="block md:hidden divide-y divide-slate-100">
              {filteredQueueItems.map((item, idx) => {
                ...
                return (
                  <div key={item.id} className="p-3.5 space-y-2 hover:bg-slate-50/70 transition-colors">
                    {/* Top: Index + Student + Status */}
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 min-w-0 flex-1">
                        <span className="text-slate-400 font-mono text-[10px]">#{idx + 1}</span>
                        <span className="font-mono font-black text-indigo-700 bg-indigo-50 px-1.5 py-0.2 rounded text-[11px] shrink-0">
                          {item.followerUsername}
                        </span>
                        <strong className="text-slate-800 text-xs truncate">{item.followerName || item.followerUsername}</strong>
                      </div>
                      <div className="shrink-0">
                        {item.status === 'RUNNING' ? (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black bg-indigo-100 text-indigo-800 border border-indigo-300 animate-pulse">
                            <RefreshCw className="w-2.5 h-2.5 animate-spin" /> Chạy
                          </span>
                        ) : item.status === 'QUEUED' ? (
                    ...
                    {/* Bottom: Action + Course + Time */}
                    <div className="flex items-center justify-between gap-2 text-[11px]">
                      <div className="flex items-center gap-1.5 min-w-0">
                        <span
                          className={`px-1.5 py-0.5 rounded-md font-bold text-[10px] shrink-0 ${actionDef.badgeStyle.bg} ${actionDef.badgeStyle.text} border ${actionDef.badgeStyle.border}`}
                        >
                          {actionDef.name}
                        </span>
                        {item.flowAction.includes('SYNC') ? (
                          <span className="text-emerald-700 font-bold truncate">Tất cả môn LT</span>
                        ) : (
                          <span className="text-slate-700 font-bold truncate">
                            {item.ten_mon || item.ma_mon || 'Môn'}
                            {item.nhom_to && <span className="text-amber-700 ml-1">(Nhóm {item.nhom_to})</span>}
                          </span>
                        )}
                      </div>
                    ...
                    {/* Result Message */}
                    {item.resultMessage && (
                      <p className="text-[10px] text-slate-500 truncate bg-slate-50 rounded-lg px-2 py-1 border border-slate-100" title={item.resultMessage}>
                        {item.resultMessage}
                      </p>
                    )}
                  </div>
            ...
            {/* Desktop Table List (Hidden on Mobile, Visible on md+) */}
